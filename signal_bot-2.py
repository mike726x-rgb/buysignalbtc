...
def notify(text, verbose=False):
    ...
    if not (token and chat):
        if verbose:
            print("  Telegram NOT configured: TELEGRAM_BOT_TOKEN and/or TELEGRAM_CHAT_ID is missing")
        return False
    try:
...
def run_once(args, state):
    ...
    for pair in args.pairs:
        try:
            ...
            print(text + "\n")
            if args.test_alert:
                notify("TEST ALERT (not a new signal)\n" + text, verbose=True)
...
def main():
    ...
    p.add_argument("--test-alert", action="store_true",
                   help="send the current status of each pair to Telegram, even without a new cross")
    args = p.parse_args()
